...
public class LicenseVertify
{
    ...
    private String confPath="/config/licenseVertifyConf.properties";
    ...
    public void install(String licdir)
    {
        try
        {
            ...
            pInfo("(LICENSE) 安装证书成功!");
        }
        catch (Exception e)
        {
            pInfo("(LICENSE) 安装证书失败!");
            e.printStackTrace();
            System.exit(0);
        }
    ...
    public int vertify()
    {
        try
        {
            ...
            pInfo("(LICENSE) 验证证书成功!");
            return 0;
        }
        catch(LicenseContentException ex)
        {
            pInfo("(LICENSE) 证书已经过期!");
            ex.printStackTrace();
            return 1;
        }
        catch (Exception e)
        {
            pInfo("(LICENSE) 验证证书失败!");
            e.printStackTrace();
            return 2;
        }
    }
